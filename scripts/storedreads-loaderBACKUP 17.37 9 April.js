...
if (executeBtn && tableWrapper && tableBody) {
  executeBtn.addEventListener("click", e => {
    ...
    if (selectedRange === "7") {
      // Filter for the last 7 days based on the date
      ...
      // Filter for the last 30 days based on the date
      rows = rows.filter(row => {
        const rowDate = parseUKDate(row.date);
        rowDate.setHours(0, 0, 0, 0);
...
function parseUKDate(dateStr) {
  const [day, month, year] = dateStr.split(".").map(Number);  // Changed to use '.' as separator
